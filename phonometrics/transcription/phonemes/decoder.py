# taken from https://github.com/jonatasgrosman/huggingsound
        ...
        for i in range(len(predicted_ids)):  # for each item in the batch
            ...
            for t in range(
                ...
                if predicted_id != self.token_set.blank_token_id:
                    ...
                    if (
                        len(i_predicted_ids) == 0
                        or previous_predicted_id
                        == self.token_set.blank_token_id
                        or predicted_id != i_predicted_ids[-1]
                    ):
                        ...
                        predicted_id == i_predicted_ids[-1]
                        and return_timesteps
                    ):
                        i_end_timesteps[-1] = t
            ...
            predictions.append(
                {
                    "ids": i_predicted_ids,
                    "start_timesteps": (
                        i_start_timesteps if return_timesteps else None
                    ),
                    "end_timesteps": (
                        i_end_timesteps if return_timesteps else None
                    ),
                }
            )
        ...
        for i, prediction in enumerate(predictions):
            ...
            for j in J:
                ...
                if "transcription" in prediction:
                    token = (
                        transcription[j]
                        if transcription[j] != " "
                        else self.token_set.silence_token
                    )
                    ...
                    if predicted_id == self.token_set.silence_token_id:
                        ...
                        and self.token_set.tokens[predicted_id]
                        in self.token_set.special_tokens
                    ):
                        continue
                    else:
                ...
                if prediction["start_timesteps"] is not None:
                    ...
                    ):  # it needs to have at least one timestep of difference
                        window_end_timestep += 1

                    window_probabilities = [
                        x[predicted_id]
                        for x in logits_probs[i][
                            start_timestep:window_end_timestep
                        ]
                    ]
            ...
            if len(transcription) > 0:
                ...
                if len(transcription_start_timestamps) > 0:
                    transcription_start_timestamps = (
                        transcription_start_timestamps[
                            left_offset : len(transcription_start_timestamps)
                            - right_offset
                        ]
                    )
...
class ParlanceLMDecoder(Decoder):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "To use this decoder please install the ctcdecoder from https://github.com/parlance/ctcdecode"
            )

        # creating the tokens forcing the silence token to be a whitespace
        tokens = [
            x if x != self.token_set.silence_token else " "
            for x in self.token_set.tokens
        ]
    ...
    def _get_predictions(self, logits: torch.Tensor) -> list[dict]:
        ...
        beam_results, beam_scores, beam_timesteps, beam_end_lens = (
            self.ctcdecoder.decode(logits.to("cpu"))
        )
...
class FlashlightLMDecoder(Decoder):
    ...
    def __init__(
        ...
        try:
            ...
            from flashlight.lib.text.decoder import LexiconDecoderOptions
            from flashlight.lib.text.decoder import LexiconFreeDecoder
            from flashlight.lib.text.decoder import LexiconFreeDecoderOptions
            from flashlight.lib.text.decoder import SmearingMode
            from flashlight.lib.text.decoder import Trie
            from flashlight.lib.text.dictionary import create_word_dict
            from flashlight.lib.text.dictionary import load_words

        except:
            raise ImportError(
                "To use this decoder please install the flashlight python bindings from https://github.com/facebookresearch/flashlight/tree/master/bindings/python"
            )
        ...
        self.beam_size_token = (
            beam_size_token
            if beam_size_token is not None
            else self.token_set.size
        )
        ...
        if self.lexicon_path is not None:
            ...
            self.trie = Trie(
                self.token_set.size, self.token_set.silence_token_id
            )
                ...
                for spelling in spellings:
                    ...
                    for token in spelling:
                        if token.upper() in tokens_keys:
                            spelling_idxs.append(
                                self.token_set.id_by_token[token.upper()]
                            )
                        elif token.lower() in tokens_keys:
                            spelling_idxs.append(
                                self.token_set.id_by_token[token.lower()]
                            )
                        else:
                            logging.warning(
                                f"WARNING: The token {token} not exist in your TokenSet, using <unk> token instead"
                            )
...
class KenshoLMDecoder(Decoder):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "To use this decoder please install the pyctcdecode from https://github.com/kensho-technologies/pyctcdecode"
            )
    ...
    def _get_predictions(self, logits: torch.Tensor) -> list[dict]:
        ...
        with Pool() as pool:
            ...
            decoder_outputs = self.decoder.decode_beams_batch(
                ...
                hotword_weight=self.hotword_weights,
            )
            ...
            for decoder_output in decoder_outputs:
                ...
                transcription, word_frames, logit_score, lm_score = (
                    decoder_output[0]
                )
                start_timesteps = []
                ...
                for word_frame in word_frames:
                    ...
                    timestep_per_char = max(
                        1, int((end_timestep - start_timestep) / len(word))
                    )
                    ...
                    for i in range(0, len(word)):
                        start_timesteps.append(
                            start_timestep + i * timestep_per_char
                        )
