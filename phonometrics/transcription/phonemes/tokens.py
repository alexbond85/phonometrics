...
class TokenSet:
    ...
    def non_special_tokens(self):
        return [
            token for token in self.tokens if token not in self.special_tokens
        ]
    ...
    def to_processor(
        self, model_name_or_path: str = "facebook/wav2vec2-large-xlsr-53"
    ):
    ...
    @classmethod
    def from_processor(
        cls, processor: Wav2Vec2Processor, letter_case: str = "lowercase"
    ):
        ...
        tokens = [
            x
            for x in processor.tokenizer.convert_ids_to_tokens(
